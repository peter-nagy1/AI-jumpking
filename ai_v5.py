...
for i in range(500):
    ...
    if not match:
        ...
        badMove = True
        while badMove:
            move = input.getRandMove()
            sz = input.getRandSize()
            
            badMove = False
                
            # Check if move is in bad moves
            for bmove in badMoves:
                if similar_images(bmove[0], preImg) and bmove[1] == move and bmove[2] == sz:
                    badMove = True
                    print("Ignored bad move!")
                    break                
...
np.save(f"history/backup/good_moves_v5_{TIME}.npy", goodMoves)
np.save(f"history/backup/bad_moves_v5_{TIME}.npy", badMoves)
